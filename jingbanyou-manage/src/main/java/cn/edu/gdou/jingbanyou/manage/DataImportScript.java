...
public class DataImportScript implements CommandLineRunner {
    ...
    private void importExcelBehavior() throws Exception {
        ...
        // 使用 SXSSF 流式读取（14万行不会OOM）
        try (FileInputStream fis = new FileInputStream(EXCEL_BEHAVIOR);
             SXSSFWorkbook workbook = new SXSSFWorkbook((XSSFWorkbook) WorkbookFactory.create(fis), 1000)) {
            ...
            while (rowIter.hasNext()) {
                ...
                if (batchInteractions.size() >= BATCH_SIZE) {
                    for (VisitorInteraction interaction : batchInteractions) {
            ...
            if (!batchInteractions.isEmpty()) {
                for (VisitorInteraction vi : batchInteractions) {
                    visitorInteractionMapper.insert(vi);
                }
                interCount.addAndGet(batchInteractions.size());
            }
        }
    ...
    private void generateOperationStats(Map<String, List<ExcelRow>> grouped, AtomicInteger statsCount) {
        ...
        for (Map.Entry<String, List<ExcelRow>> entry : grouped.entrySet()) {
            ...
            // 平均停留时长
            double avgStay = rows.stream()
                    .filter(r -> r.stayDuration != null)
                    .mapToDouble(r -> r.stayDuration)
                    .average().orElse(0);
            ...
            // 情感分布
            long positive = rows.stream().filter(r -> "positive".equals(detectEmotion(r.satisfaction))).count();
            long neutral  = rows.stream().filter(r -> "neutral".equals(detectEmotion(r.satisfaction))).count();
            long negative = rows.stream().filter(r -> "negative".equals(detectEmotion(r.satisfaction))).count();

            Map<String, Long> emotionDist = Map.of(
                    "positive", positive,
                    "neutral", neutral,
                    "negative", negative
            );
            ...
            try {
                ...
                operationStatsMapper.insert(stats);
