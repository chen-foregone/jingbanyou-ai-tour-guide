-- ====================== 景区导览 AI 数字人 - 后台管理数据库表结构 ======================
-- 字符集：utf8mb4（支持表情、特殊字符），排序规则：utf8mb4_general_ci
-- 优化版本：v1.1 (补充 RAG、向量化、会话追踪等字段)

CREATE DATABASE IF NOT EXISTS scenic_ai_digital_human DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;
USE scenic_ai_digital_human;
...
CREATE TABLE scenic_spot (
    ...
    create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    update_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (scenic_id) REFERENCES scenic_area(id) ON DELETE CASCADE,
...
CREATE TABLE knowledge_doc (
    ...
    update_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (scenic_id) REFERENCES scenic_area(id) ON DELETE CASCADE,
    FOREIGN KEY (creator) REFERENCES sys_admin(id),
    FOREIGN KEY (updater) REFERENCES sys_admin(id),
    INDEX idx_scenic_id (scenic_id),
...
CREATE TABLE faq (
    ...
    update_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (scenic_id) REFERENCES scenic_area(id) ON DELETE CASCADE,
    FOREIGN KEY (spot_id) REFERENCES scenic_spot(id) ON DELETE SET NULL,
    INDEX idx_scenic_id (scenic_id),
...
DROP TABLE IF EXISTS digital_human_config;
CREATE TABLE digital_human_config (
    ...
    update_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (scenic_id) REFERENCES scenic_area(id) ON DELETE CASCADE,
    UNIQUE KEY uk_scenic_default (scenic_id, is_default) WHERE is_default=1,
    INDEX idx_scenic_id (scenic_id),
    INDEX idx_human_type (human_type)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='AI 数字人形象配置表';

-- ====================== 8. 数字人动作库表（表情/手势/肢体） ======================
DROP TABLE IF EXISTS digital_human_action;
CREATE TABLE digital_human_action (
...
-- ====================== 10. 路线 - 景点关联表（多对多） ======================
DROP TABLE IF EXISTS route_spot_relation;
CREATE TABLE route_spot_relation (
    ...
    create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (route_id) REFERENCES tour_route(id) ON DELETE CASCADE,
...
CREATE TABLE visitor_interaction (
    ...
    FOREIGN KEY (human_id) REFERENCES digital_human_config(id),
    INDEX idx_scenic_id (scenic_id),
...
CREATE TABLE visitor_analysis (
    ...
    satisfaction_trend VARCHAR(50) COMMENT '满意度趋势 rising/stable/falling',
    hot_questions JSON COMMENT '热门问题 TOP10',
    complaint_topics JSON COMMENT '投诉话题统计',
    suggestion_summary TEXT COMMENT '建议汇总',
    service_suggest TEXT COMMENT '服务优化建议',
    ai_generated TINYINT DEFAULT 0 COMMENT '是否 AI 生成报告 0-人工 1-AI',
    report_file VARCHAR(255) COMMENT '报告文件地址',
    creator BIGINT COMMENT '创建人',
    create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    update_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (scenic_id) REFERENCES scenic_area(id),
...
CREATE TABLE sys_operation_log (
    ...
    admin_id BIGINT COMMENT '操作员 ID',
    operation_type VARCHAR(50) NOT NULL COMMENT '操作类型 create/update/delete/query/export',
...
-- 插入系统配置初始值
INSERT INTO sys_config (config_key, config_value, config_type, config_group, config_desc) VALUES
