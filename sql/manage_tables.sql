...
CREATE TABLE `manage_scenic_area` (
    ...
    `top_features` JSON COMMENT '首屏功能亮点文案数组，如 ["智能导览","路线规划","语音问答"]',
    `quick_prompts` JSON COMMENT '快捷提问文案数组，如 ["第一次来怎么安排路线？","几点开门？"]',
...
CREATE TABLE `manage_knowledge_doc` (
    ...
    `creator` BIGINT COMMENT '创建人 (关联 sys_user.user_id)',
    `updater` BIGINT COMMENT '最后更新人 (关联 sys_user.user_id)',
...
CREATE TABLE `manage_digital_human_config` (
    ...
    `avatar_image` VARCHAR(255) COMMENT '人物头像图片地址',
    `default_greeting` TEXT COMMENT '默认问候语',
    `model_json_url` VARCHAR(500) COMMENT 'Live2D 模型 .model3.json 可访问地址（前端加载模型必须）',
    `landscape_height` DECIMAL(4,2) DEFAULT 1.50 COMMENT '横屏模型显示高度',
    `portrait_height` DECIMAL(4,2) DEFAULT 1.60 COMMENT '竖屏模型显示高度',
    `scale_x` DECIMAL(4,2) DEFAULT 2.00 COMMENT 'X 方向缩放',
    `offset_x` DECIMAL(4,2) DEFAULT 1.20 COMMENT 'X 方向偏移',
    `offset_y` DECIMAL(4,2) DEFAULT 0.50 COMMENT 'Y 方向偏移',
    `idle_motion_group` VARCHAR(50) DEFAULT 'Idle' COMMENT '空闲动作组名',
    `tap_motion_group` VARCHAR(50) DEFAULT 'Tap' COMMENT '点击动作组名',
    `tts_voice_code` VARCHAR(50) COMMENT 'TTS 音色代码',
    `sample_audio_url` VARCHAR(500) COMMENT '音色试听音频地址',
    `appearance_config` JSON COMMENT '外观扩展配置 (JSON)',
    `voice_config` JSON COMMENT '语音合成扩展参数 (TTS 模型/语速/音调 JSON)',
...
CREATE TABLE `manage_tour_route` (
    ...
    `route_tags` JSON COMMENT '路线标签数组，如 ["首次来访","轻徒步","观景核心线"]，前端卡片直接展示',
